...
func populate_windows():
	for w in wContainer.get_children():
		if w.get_child(0) is wTitlebar:
			...
			for b in pContainer.get_children():
				if b is wButton and b.assingedWindow == w:
...
func _populate_pill_connections():
	for p in pContainer.get_children():
		if p is wButton:
			if !p.btn_up.is_connected(minimize_or_restore_window):
...
func minimize_or_restore_window(btnSender:wButton):
	...
	wContainer.minimize_restore_or_focus_window(btnSender.assingedWindow)
	pass
