...
enum ICONTYPE {FILE, FOLDER, APP_DECODER, APP_LOG, APP_MSN, APP_FORM, APP_TIC}
...
var icon_xox = load("res://Sprites/joystick.png")
...
func setup_icon(icon_label = "IconName", file_icon = null):
	...
	if icon_label.containsn("TIC-TAC"):
		$TextureRect.texture = icon_xox
	pass
...
func open_folder():
	...
	# Check if this icon is inside a window (not on Desktop)
	var parent_window := _find_parent_window()
	if parent_window != null and parent_window is window_ls:
		# We're inside a window - reload it with the new folder path
		parent_window.load_ls(file_path)
		# Bring the window to top
		var wm : ManagerWindows = get_tree().get_first_node_in_group("window_manager")
		if wm != null:
			wm.move_window_to_top(parent_window)
	else:
		# We're on Desktop - create a new window
		var wm : ManagerWindows = get_tree().get_first_node_in_group("window_manager")
		if wm != null:
			wm.create_new_folder_window(file_path)

func _find_parent_window() -> Node:
	# Walk up the parent tree to find a window_ls node
	var current := get_parent()
	while current != null:
		if current is window_ls:
			return current
...
func open_ttt():
	var wm : ManagerWindows = get_tree().get_first_node_in_group("window_manager")
	if (wm != null): wm.open_or_focus_ttt()
